...
@section('css')
    ...
    <style>
        ...
        .btn-edit, #modal-edit{display: none;}
    </style>
...
@section('content')
    ...
    <form id="inappAdsForm" enctype="multipart/form-data">
    ...
    <script>
        $(function () {
            ...
            $('#data-table').DataTable({
                ...
                ajax: "{{ route('inapp-ads.index') }}",
                columns: [
                    {data: 'checkbox', name: 'checkbox', orderable:false,searchable:false},
                    {data: 'image_url', name: 'image_url', render: function(data, row){
                        if(data){
                            return `<img src="${data}" class="w-50 h-25 rounded">`
                        }

                        let baseUrl = "{{ env('APP_URL') }}"

                        return `<img src="${baseUrl}/img/placeholder.jpg" class="w-50 h-25 rounded">`
                    }},
            ...
            $('#inappAdsForm').on('change', '.fileInput', function(e){
            ...
            $('#inappAdsForm').on('click', '.img', function(e){
                $(this).prev().click();
            ...
            $('#inappAdsForm').on('click', '.removeImage', function(e){
            ...
            // Create Categories
            $('#inappAdsForm').submit(function(e){
                ...
                if($('#items').children().length < 1){
                    $('#inappAdsForm #addInfoText').addClass('animate__animated animate__bounce animate__delay-0.5s');

                    setTimeout(() => {
                        $('#inappAdsForm #addInfoText').removeClass('animate__animated animate__bounce animate__delay-0.5s')
                    }, 500)
                ...
                const image = [...$('#inappAdsForm .fileInput')];

                $('#inappAdsForm .title').each(function(index, e){
            ...
            // Delte In App Ads
            function deleteInAppAds(){
                setUpAjax();

                // If Yes
                $.ajax({
                    url: "{{ route('inapp-ads.bulkDelete') }}",
                    type: 'POST',
                    data: {ids: IDS},
                    success: function (data) {
                        if (data['status']) {
